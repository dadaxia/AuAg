...
import com.gold.cn.service.impl.AgServiceImpl;
...
public class AgController {
    @Autowired
    private AgServiceImpl agService;

    @RequestMapping(value = "/getNowData", method = RequestMethod.GET, produces = {"application/json;charset=UTF-8"})
    @ResponseBody
    public Ag getData(){
        if(ScanAuAgUSDSchedule.agList != null && ScanAuAgUSDSchedule.agList.size() > 0){
            return ScanAuAgUSDSchedule.agList.get(0);
        }
        return null;
    }
    ...
    @ResponseBody
    public List<Ag> getLatestAgData(){
        ...
        return agService.getLatestAgData(timestamp.valueOf(str));
    }
}
